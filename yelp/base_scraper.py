...
class Scraper:
    ...
    def response_queue_monitor(self):
        logging.info("Started response queue")
        self.init_db()
        while True:
            try:
...
def test_scraper():
    def query_generator():
        for i in range(100):
            yield {"url": "https://www.example.com/", "params": ""}

    def scraper(data_dict: dict):
        resp = requests.get(data_dict["url"])
        yield {"data": resp.text[:20], "url": resp.url}

    dummy_data = {"data": "", "url": ""}

    s = Scraper(
        file_name="sqlite/test.db",
        num_workers=10,
        query_generator=query_generator,
        query_scraper=scraper,
